/**
 * Comprehensive Abuse Test Suite for Shotstack Edit Schema
 *
 * This test suite exhaustively tests every property, object, array validation
 * to find edge cases, malformed data handling, and boundary conditions.
 */

const { editSchema, outputSchema, timelineSchema } = require('../dist/zod/zod.gen.cjs');
...
function expectValid(schema, data, description) {
  ...
  if (!result.success) {
    throw new Error(`Expected valid but got: ${JSON.stringify(result.error.issues.map(i => i.message))}`);
  }
}
...
// Helper to create a valid minimal edit
function createMinimalEdit(assetOverride = {}) {
  return {
    timeline: {
      tracks: [
        {
          clips: [
            {
              asset: { type: 'video', src: 'https://example.com/video.mp4', ...assetOverride },
              start: 0,
              length: 5
            }
          ]
...
function createMinimalClip(assetOverride = {}) {
  return {
    asset: { type: 'video', src: 'https://example.com/video.mp4', ...assetOverride },
    start: 0,
    length: 5
...
test('Reject timeline with empty tracks array', () => {
  expectInvalid(editSchema, { timeline: { tracks: [] }, output: { format: 'mp4' } });
});

test('Reject track with empty clips array', () => {
  expectInvalid(editSchema, {
...
test('Accept valid video asset', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'video', src: 'https://example.com/video.mp4' }));
});
...
test('Reject video asset with missing src', () => {
  ...
  delete edit.timeline.tracks[0].clips[0].asset.src;
...
test('Accept valid image asset', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'image', src: 'https://example.com/image.jpg' }));
});
...
test('Accept valid audio asset', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'audio', src: 'https://example.com/audio.mp3' }));
});
...
test('Accept valid text asset', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'text', text: 'Hello World' }));
});
...
test('Accept valid rich-text asset', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'rich-text', text: 'Hello World' }));
});
...
test('Accept valid html asset', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'html', html: '<p>Hello</p>' }));
});
...
test('Accept valid caption asset', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'caption', src: 'https://example.com/captions.srt' }));
});
...
test('Accept valid luma asset', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'luma', src: 'https://example.com/mask.mp4' }));
});
...
test('Accept valid title asset', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'title', text: 'My Title' }));
});

test('Reject title asset with missing text', () => {
  expectInvalid(editSchema, createMinimalEdit({ type: 'title' }));
});

// Shape Asset
test('Accept valid rectangle shape asset', () => {
  expectValid(editSchema, createMinimalEdit({
    type: 'shape',
    shape: 'rectangle',
...
test('Reject shape asset with invalid shape enum', () => {
  expectInvalid(editSchema, createMinimalEdit({ type: 'shape', shape: 'triangle' }));
});

// Text-to-Image Asset
test('Accept valid text-to-image asset', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'text-to-image', prompt: 'A beautiful sunset' }));
});
...
test('Accept valid image-to-video asset', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'image-to-video', src: 'https://example.com/image.jpg' }));
});

test('Reject image-to-video with empty src', () => {
  expectInvalid(editSchema, createMinimalEdit({ type: 'image-to-video', src: '' }));
});

// Invalid Asset Type
test('Reject unknown asset type', () => {
  expectInvalid(editSchema, createMinimalEdit({ type: 'unknown-type', src: 'https://example.com/file.mp4' }));
});
...
test('Accept speed = 0 (minimum)', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'video', src: 'https://x.com/v.mp4', speed: 0 }));
});

test('Accept speed = 10 (maximum)', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'video', src: 'https://x.com/v.mp4', speed: 10 }));
});

test('Reject speed > 10', () => {
  expectInvalid(editSchema, createMinimalEdit({ type: 'video', src: 'https://x.com/v.mp4', speed: 11 }));
});

test('Reject negative speed', () => {
  expectInvalid(editSchema, createMinimalEdit({ type: 'video', src: 'https://x.com/v.mp4', speed: -1 }));
});

// Volume
test('Accept volume = 0 (muted)', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'video', src: 'https://x.com/v.mp4', volume: 0 }));
});

test('Accept volume = 1 (full)', () => {
  expectValid(editSchema, createMinimalEdit({ type: 'video', src: 'https://x.com/v.mp4', volume: 1 }));
});

test('Reject volume > 1', () => {
  expectInvalid(editSchema, createMinimalEdit({ type: 'video', src: 'https://x.com/v.mp4', volume: 1.5 }));
});

test('Reject negative volume', () => {
  expectInvalid(editSchema, createMinimalEdit({ type: 'video', src: 'https://x.com/v.mp4', volume: -0.5 }));
});
...
test('Reject crop values > 1', () => {
  expectInvalid(editSchema, createMinimalEdit({
    type: 'video', src: 'https://x.com/v.mp4',
    crop: { top: 1.5 }
  }));
});
...
test('Accept shape = rectangle', () => {
  expectValid(editSchema, createMinimalEdit({
    type: 'shape', shape: 'rectangle', rectangle: { width: 100, height: 100 }
  }));
});

test('Accept shape = circle', () => {
  expectValid(editSchema, createMinimalEdit({
    type: 'shape', shape: 'circle', circle: { radius: 50 }
  }));
});

test('Accept shape = line', () => {
  expectValid(editSchema, createMinimalEdit({
    type: 'shape', shape: 'line', line: { length: 100, thickness: 4 }
  }));
});
...
test('Accept multiple tracks', () => {
  ...
  edit.timeline.tracks.push({
    clips: [{ asset: { type: 'image', src: 'https://x.com/i.jpg' }, start: 0, length: 5 }]
  });
  expectValid(editSchema, edit);
});
...
test('Accept empty crop object', () => {
  expectValid(editSchema, createMinimalEdit({
    type: 'video', src: 'https://x.com/v.mp4', crop: {}
  }));
});
...
test('Accept scale as array', () => {
  const edit = createMinimalEdit();
  edit.timeline.tracks[0].clips[0].scale = [{ from: 0.5, to: 1, start: 0, length: 2 }];
  expectValid(editSchema, edit);
});
...
test('Accept opacity as array', () => {
  const edit = createMinimalEdit();
  edit.timeline.tracks[0].clips[0].opacity = [{ from: 0, to: 1, start: 0, length: 2 }];
  expectValid(editSchema, edit);
});
...
test('Accept volume as number', () => {
  expectValid(editSchema, createMinimalEdit({
    type: 'video', src: 'https://x.com/v.mp4', volume: 0.5
  }));
});
...
test('Accept rich-text padding as number', () => {
  expectValid(editSchema, createMinimalEdit({
    type: 'rich-text', text: 'Hello', padding: 10
  }));
});

test('Accept rich-text padding as object', () => {
  expectValid(editSchema, createMinimalEdit({
    type: 'rich-text', text: 'Hello',
...
test('Reject S3 destination without required region', () => {
  const edit = createMinimalEdit();
  edit.output.destinations = [{
    provider: 's3',
...
test('Reject S3 destination without required bucket', () => {
  const edit = createMinimalEdit();
  edit.output.destinations = [{
    provider: 's3',
...
test('Accept Mux destination with options', () => {
  ...
  edit.output.destinations = [{
    provider: 'mux',
    options: { playbackPolicy: ['public', 'signed'] }
  }];
  expectValid(editSchema, edit);
});

// Google Cloud Storage destination
test('Accept GCS destination with required options', () => {
  const edit = createMinimalEdit();
  edit.output.destinations = [{
...
test('Reject GCS destination without bucket', () => {
  const edit = createMinimalEdit();
  edit.output.destinations = [{
    provider: 'google-cloud-storage',
    options: {}
...
test('Accept Vimeo destination with options', () => {
  ...
  edit.output.destinations = [{
    ...
    options: {
      ...
      privacy: { view: 'anybody', embed: 'public', download: true, add: false }
    }
  }];
  expectValid(editSchema, edit);
});
...
// Extra/unknown properties (additionalProperties)
test('Handle extra properties at top level', () => {
  const edit = createMinimalEdit();
  edit.unknownProperty = 'value';
  // Depending on schema strictness, this may pass or fail
  // We're just checking it doesn't crash
  const result = editSchema.safeParse(edit);
  console.log(`    (additionalProperties result: ${result.success ? 'allowed' : 'rejected'})`);
  passCount++;
});

test('Handle extra properties on asset', () => {
  const edit = createMinimalEdit({ type: 'video', src: 'https://x.com/v.mp4', unknownProp: 'value' });
  const result = editSchema.safeParse(edit);
  console.log(`    (additionalProperties on asset result: ${result.success ? 'allowed' : 'rejected'})`);
  passCount++;
});
...
test('Accept soundtrack with effect', () => {
  const edit = createMinimalEdit();
  edit.timeline.soundtrack = { src: 'https://example.com/music.mp3', effect: 'fadeIn' };
  expectValid(editSchema, edit);
});

test('Accept soundtrack with fadeInFadeOut effect', () => {
  const edit = createMinimalEdit();
  edit.timeline.soundtrack = { src: 'https://example.com/music.mp3', effect: 'fadeInFadeOut' };
  expectValid(editSchema, edit);
});

test('Reject soundtrack with invalid effect', () => {
  const edit = createMinimalEdit();
  edit.timeline.soundtrack = { src: 'https://example.com/music.mp3', effect: 'reverse' };
  expectInvalid(editSchema, edit);
});

test('Accept soundtrack with volume', () => {
  const edit = createMinimalEdit();
  edit.timeline.soundtrack = { src: 'https://example.com/music.mp3', volume: 0.5 };
  expectValid(editSchema, edit);
});
...
test('Accept fully featured video edit', () => {
  const edit = {
    timeline: {
      background: '#000000',
      soundtrack: { src: 'https://example.com/music.mp3', volume: 0.8, effect: 'fadeInFadeOut' },
    ...
    timeline: {
      tracks: [
          ...
          clips: [
            {
              asset: {
                ...
                style: { letterSpacing: 2, lineHeight: 1.5, textTransform: 'uppercase' },
        ...
        },
        {
          clips: [
            {
              asset: { type: 'image', src: 'https://example.com/background.jpg' },
              start: 0,
              length: 5,
              fit: 'cover'
            }
          ...
          clips: [
            {
              asset: {
                type: 'shape',
                shape: 'rectangle',
                rectangle: { width: 400, height: 400, cornerRadius: 20 },
